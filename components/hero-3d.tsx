...
export default function Hero3D() {
  ...
  if (!mounted) {
    return <div className="absolute inset-0 w-full h-full bg-gradient-to-br" />
  }

  return (
    <div className="absolute left-0 top-0 h-full w-[70%] ml-2">
      <Canvas camera={{ position: [-2, 0, 10], fov: 75 }}>
        <ambientLight intensity={0.4} />
        <pointLight position={[10, 10, 10]} intensity={0.8} color="lightblue" />
        <pointLight position={[-10, -10, -10]} intensity={0.4} color="#lightblue" />
        <directionalLight position={[5, 5, 5]} intensity={0.6} color="white" />
